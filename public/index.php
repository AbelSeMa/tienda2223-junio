...
<body>
    ...
    $pdo = conectar();
    $sent = $pdo->query("SELECT a.*, d.descuento FROM articulos AS a
                                  LEFT JOIN descuentos AS d ON a.descuento = d.id 
                                  ORDER BY a.descuento");
                ...
                <?php foreach ($res as $fila) : ?>
                    <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <?php if ($fila['descuento'] != null) : ?>                         
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-green-200">Descuento: <?= hh($fila['descuento']) ?></p>
                        <?php endif ?>
                        <?php if ($fila['stock'] > 0): ?>
